...
function RecordAnswerSection({ mockInterviewQuestion, activeQuestionIndex, interviewData }) {
  ...
  const UpdateUserAnswer = async () => {
    ...
    const result = await generateInterviewQuestions(feedbackPrompt);
    const mockJsonResp = JSON.stringify(result);
    console.log(mockJsonResp);
  const feedbackData = JSON.parse(result[0]?.answer.replace('"1/5"', '1') || "{}");
